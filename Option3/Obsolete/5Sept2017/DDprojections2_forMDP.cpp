...
// USAGE DD_Option3Projections_forMDP Results/DelayDifferenceModelParameters.csv Data/WeeklyPercentageOfSpawners.txt Results/LinearizedBevertonAndHoltCoef Data/AverageEffortPattern.txt Data/Availability.txt 100

// RESULTS in file Results/Simulation/Projections_forMDP.txt
...
int Projections2_forMDP(const long unsigned int max_timestep, const std::vector<double> &NontargetedEffort, const std::vector<Parameter> &par, std::vector<double> &PropMature, std::vector<double> &SrPar, std::vector<double> &FishingPattern, std::vector<double> &Availability);
...
MaturityFile.close();
    ...
    Projections2_forMDP(NbYear*NIPY, NontargetedEffort, ModelInputParameters, PropMature, SrPar, FishingPattern, Availability);
