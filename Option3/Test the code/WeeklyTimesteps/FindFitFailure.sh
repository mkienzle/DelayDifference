...
for i in `seq 1 $maxSim`
 do 
    ...
    if grep -q "Natural mortality || limited ||    0.019" Results/${args[0]}/$i/FitOutcome.txt
    then echo $i >> $OUTPUT
    fi

    if grep -q "Natural mortality || limited ||    0.020" Results/${args[0]}/$i/FitOutcome.txt
    then echo $i >> $OUTPUT
    fi

    if grep -q "Natural mortality || limited ||    0.069" Results/${args[0]}/$i/FitOutcome.txt
    then echo $i >> $OUTPUT
    fi

    if grep -q "Natural mortality || limited ||    0.070" Results/${args[0]}/$i/FitOutcome.txt
    then echo $i >> $OUTPUT
    fi
