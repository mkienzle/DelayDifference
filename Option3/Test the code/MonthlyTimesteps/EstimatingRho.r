...
K.m <- 0.0528 * 52 / 12; K.f <- 0.0436 * 52 / 12; # per week
...
length.at.age <- data.frame(Female = rep(NA,max.age.group), Male = rep(NA,max.age.group)); dimnames(length.at.age)[[1]] <- seq(1,max.age.group);
length.at.age$Female <- vbgf(seq(1,max.age.group), Linf.f, K.f, t0 = 0)
length.at.age$Male <- vbgf(seq(1,max.age.group), Linf.m, K.m, t0 = 0)

# Weight at age
weight.at.age <- data.frame(Female = rep(NA,max.age.group), Male = rep(NA,max.age.group)); dimnames(weight.at.age)[[1]] <- seq(1,max.age.group);
...
# Plot weight at age
plot(seq(1,max.age.group), weight.at.age$Female, pch = 1, xlab = "Time (weeks)", ylab = "Weight (g)", las = 1)
points( seq(1,max.age.group), weight.at.age$Male, pch = 2)
points(seq(1,max.age.group), average.weight.at.age, pch = 19)

curve( vbgf(x, coef(nls.model)[1], coef(nls.model)[2], coef(nls.model)[3]), from = rec.age, to = max.age.group, lwd = 2, col = "blue", add = TRUE)
#curve( Eq114(Schnute.results.for.TigerPrawn$par, x, k=rec.age), from = rec.age, to = max.age.group, lwd = 2, col = "grey", add = TRUE)
curve( Eq114.bis(Schnute.results.for.TigerPrawn.2par$par, x, k=rec.age), from = rec.age, to = max.age.group, lwd = 2, col = "red", add = TRUE)
#curve( Eq114.tris(Schnute.results.for.TigerPrawn.1par$minimum, x, k=rec.age, rho = exp(-0.1794067)), from = rec.age, to = max.age.group, lwd = 2, col = "green", add = TRUE)
...
ssq.Deriso.Eq <- function(par){
 sum((average.weight.at.age[seq(rec.age+1, max.age.group)] - Deriso.W(par, seq(rec.age+1, max.age.group), weight.at.recruitment = average.weight.at.age[rec.age],
 age.at.recruitment = rec.age))^2)
}
Deriso.opt <- optimize(ssq.Deriso.Eq, lower = 0.1, upper = 0.99)

curve(Deriso.W(rho =  Deriso.opt$minimum, x,  average.weight.at.age[rec.age], rec.age), from = rec.age, to =max.age.group, col = "yellow", lwd = 2, add = T)
