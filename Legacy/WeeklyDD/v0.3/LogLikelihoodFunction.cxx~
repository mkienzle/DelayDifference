...
namespace ROOT {
   ...
   namespace Minuit2 {
     ...
     double LogLikelihoodFunction::operator()(const std::vector<double>& par) const {
       ...
       sigma = par[3]; // The Std. dev. of the Gaussian distribution of predicted catch around observed catch (sigma)

       // Calculate biomasses according to the delay difference model
       std::vector<double> Biomass(fCatchValues.size(), 0.0);
       WeeklyDD(fTargetedEffortValues, fNontargetedEffortValues, Biomass, par);

       // Calculate fishing mortality and estimated catch by delay difference model
       std::vector<double> FishingMortality(fCatchValues.size(), 0.0), EstimatedCatch(fCatchValues.size(), 0.0);
