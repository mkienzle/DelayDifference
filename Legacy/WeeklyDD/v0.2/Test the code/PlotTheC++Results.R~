...
failed.minimization <- c(12,13,23,28,30,35,38,42,44,55,65,67,68,70,76,81,84,85,98,100)
...
sim.data <- read.table(paste("Results/C++Version/", i, "/SimData4.txt", sep = ""))

sim.par <- read.table(paste("Results/C++Version/", i, "/SimPar.txt", sep = ""))
sim.results$Sim.targeted.q[i] <- sim.par$V1[1]
sim.results$Sim.vm.mean[i] <- sim.par$V1[2]
sim.results$Sim.vm.sigma[i] <- sim.par$V1[3]

par.est <- read.csv(paste("Results/C++Version/", i, "/ParameterEstimates.txt", sep = ""), header = FALSE)
...
##### Biomass
print(i)
sim.biomass <- read.csv(paste("Results/C++Version/", i, "/SimulatedBiomass.csv", sep = ""), header = TRUE)
