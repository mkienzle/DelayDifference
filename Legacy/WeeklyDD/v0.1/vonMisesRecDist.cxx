...
std::vector<double> vonMisesRecDist(double a, double b){
  ...
  // Assume a year is made of 52 weeks
  
  // divide the range [-pi;pi] into 52 intervals
  vector<double> boundaries(NWPY+1, 0.0);
  for(unsigned int i = 0; i < NWPY + 1; ++i){
    boundaries[i] = -M_PI + i * 2 * M_PI / NWPY;
      }
  ...
  vector<double> intervals(NWPY, 0.0);
