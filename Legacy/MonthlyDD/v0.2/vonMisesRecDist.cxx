...
std::vector<double> vonMisesRecDist(double a, double b){
  ...
  // Assume a year is made of 12 months
  
  // divide the range [-pi;pi] into 12 intervals
  vector<double> boundaries(NMPY+1, 0.0);
  for(unsigned int i = 0; i < NMPY + 1; ++i){
    boundaries[i] = -M_PI + i * 2 * M_PI / NMPY;
      }
  ...
  vector<double> intervals(NMPY, 0.0);

  double total = 0.0;

  //std::cout << "And the value at M_PI is " << von_mises_cdf(M_PI, 0, 1) << "\n";

    for(unsigned int i = 0; i < NMPY; ++i){
