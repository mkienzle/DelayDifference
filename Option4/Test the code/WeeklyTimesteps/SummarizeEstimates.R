...
tmp.NatMort2.xtable <- as.matrix(tmp.NatMort2[,-1])
dimnames(tmp.NatMort2.xtable)[[1]] <- paste("White noise level ", substr(tmp.NatMort[,1], 16, 20), "%")
tmp.NatMort2.xtable <- xtable(tmp.NatMort2.xtable, caption = "Quantiles of the distribution of relative discrepancy between estimated and simulated natural mortality ($|\\hat{M} - M|/M$) for varying levels of random error (in rows) applied to simulated catch and effort.", label = "tab:SummaryOfDistributionOfNaturalMortality2")
print(tmp.NatMort2.xtable, file = "Results/Tables/SummaryOfDistributionOfNaturalMortality2.tex", floating.environment = 'sidewaystable')
