...
%pred_cpue = catchability_q * effort(105:end) ./ (catchability_q * effort(105:end) + M) .* Biomass(105:end) .* (1 - exp(- catchability_q * effort(105:end) - M)) ./ effort(105:end);

% Corresponding CPUEs
%cpue_butfirst105 = cpue(105:timesteps);
...
%diff = log(cpue_butfirst105) - lnpred;

% Assuming square root of catch are distributed as a Gaussian with mean square root of catch 
%diff = cpue_butfirst105 - pred_cpue;
%diff = sqrt(catch_butfirst105) - sqrt(pred_catch);
