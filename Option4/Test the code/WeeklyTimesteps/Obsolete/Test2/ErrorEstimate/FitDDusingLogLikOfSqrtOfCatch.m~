...
function negLL = EKPLogLikOfSqrtOfCatch(par, summation)
...
pred_catch = catchability_q * effort(521:end) ./ (catchability_q * effort(521:end) + M) .* Biomass(521:end) .* (1 - exp(- catchability_q * effort(521:end) - M));

% Corresponding catches
catch_butfirst520 = ctch(521:timesteps);

% Assuming square root of catch are distributed as a Gaussian with mean square root of catch 
diff = log(catch_butfirst520) - log(pred_catch);
