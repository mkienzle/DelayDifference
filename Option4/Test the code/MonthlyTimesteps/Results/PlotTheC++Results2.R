...
failed.minimization <- c(1,5,seq(10,100))
...
sim.par <- read.csv(paste("Results/", i, "/SimPar.txt", sep = ""), header = F)
...
for(i in setdiff(1:nb.sim, failed.minimization)) with(sim.results, text( Sim.NatMort, Est.NatMort, pch = i))
abline(0,1, lty = 3)
