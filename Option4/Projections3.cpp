...
int Projections2(const long unsigned int max_timestep, const double &TotTargetedEffort, const std::vector<double> &NontargetedEffort, const std::vector<double> &par, std::vector<double> &PropMature, std::vector<double> &SrPar, std::vector<double> &FishingPattern, std::vector<double> &Availability){
  ...
  int N_par = par.size(); // There are 7 fixed parameters (natural mortality, targeted q, non-targeted q, sigma, 
                          // biomass1, biomass2, vm_sigm and then magnitude of recruitment for each year and timing of recruitment for each
                          // year
  //double vm_mean = par[6]; // make sure it varies between -M_PI and +M_PI
  double vm_mean = 0.0;
  for(unsigned int i=0; i < (N_par - 7)/2; i += 1){
    vm_mean += par[7 + (N_par - 7)/2 + i];}
  vm_mean = vm_mean / ((N_par - 7)/2);

  std::cout << "Mean timing of recruitment is " << vm_mean << "\n";
