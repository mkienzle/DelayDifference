...
# MODIFIED 27 May 2014

# PURPOSE display the results of the simulation test of the weekly delay difference model written in C++

# MODIFICATIONS 2014-05-27 account for the fact that von Mises mean is estimated within [-Inf; Inf] but constrained to [-pi;pi] using modulo
nb.sim <- 100
...
sim.results$Sim.vm.mean[i] <- sim.par$V1[2]
sim.results$Sim.vm.sigma[i] <- sim.par$V1[3]
...
sim.results$Est.vm.mean[i] <- ifelse(par.est$V2[6]<0,-1,1) * abs(par.est$V2[6]) %% pi
...
for(i in setdiff(1:nb.sim, failed.minimization)) with(sim.results, text( Sim.targeted.q, Est.targeted.q, pch = i))
abline(0,1, lty = 3)
...
with(sim.results, plot(1e-6 * Sim.B1, 1e-6 * Est.B1, main = "Biomass(1) and biomass(2)", xlab = "Simulated (x1000 tonnes)", ylab = "Estimated (+- 2 SD) (x1000 tonnes)", pch = 19, type = "p", las = 1, cex = 0.2))
abline(0,1, lty = 3)
